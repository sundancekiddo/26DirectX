 ...
 * 컴파일된 중간 데이터(Blob)를 반드시 Release() 해야 메모리 누수가 없음!
 ...
 확장자가 중요한게 아님. .txt 확장자로 해서 String 읽어와서 넣어도 동작함.
...
// 셰이더 수동으로 빌드함. 그래서 VisualStudio에서 빌드 참여 안함.
// 장점: 런타임에 수정 가능함.
...
HRESULT CompileShader(const void* pSrc, bool isFile, LPCSTR szEntry, LPCSTR szTarget, ID3DBlob** ppBlob) {
    ...
    if (isFile) {
        // 방식 2, 3, 4: 파일로부터 컴파일
        hr = D3DCompileFromFile((LPCWSTR)pSrc, nullptr, nullptr, szEntry, szTarget, 0, 0, ppBlob, &pErrorBlob);//pErrorBlob에 에러 저장되어있음
    }
    else {
