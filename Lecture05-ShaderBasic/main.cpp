...
[ CPU (C++ Code) ] --- (데이터 전송: Vertex Buffer, Constant Buffer) ---+
                                    ...
                                    +----------------------+----------------------------------------------------+
                                                                        |   Pipelines and Shaders with Direct3D 12 문서 보면 VS하고 Rasterizer는 다른 과정.
                                                                            Rasterizer는 고정되어있음 그래서 VS는 인풋을 받아서 아웃풋으로 PS INPUT을 가진다고 생각해야함.
                                                                        v
                                    +----------------------+----------------------------------------------------+
                                    ...
                                    +----------------------+----------------------------------------------------+
                                                                        ...
                                                                        v
                                    +----------------------+----------------------------------------------------+
                                    |  [4. Pixel Shader (PS)] - 색상 처리 (채색) <--- **프로그래밍 가능**       |Fragment Shader(OpenGL)
                                    |  - 결정된 각 픽셀의 최종 색상을 계산함.                                   | 이름만 이런거지 Pixel에 매핑되는게 아니라 Fragment에 매핑됨;
                                    |  - 조명(Lighting), 텍스처(Texture), 그림자 효과를 여기서 처리함.          |
                                    +----------------------+----------------------------------------------------+
                                                                        |
                                                                        v
                                    +----------------------+----------------------------------------------------+
                                    |  [5. Output Merger (OM)] - 최종 병합/출력                                 |+스텐실 버퍼, Depth Buffer도 여기서;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    ================================================================================
     ...
     2. 벡터 타입 (Vector Types) - "GPU의 핵심, 한 번에 계산하기"
        ...
        - [팁]: float4 v; 선언 후 v.xyz 혹은 v.rgb 처럼 원하는 값만 골라 쓰는-> xyzw, rgba 둘 다 쓸수있음.
    ...
    */
    ...
    const char* shaderSource = R"(
        ...
        // - 특징: 삼각형 내부를 채우는 수만 개의 픽셀 수만큼 실행됨 (가장 연산량이 많음). 민수) 이 메인함수가 1억번 돌아감! 쓰레드가 1억개 생기고 쓰레드 하나마다 요 메인 함수 실행됨!
        // -----------------------------------------------------------------------------
